...
class KriptaAES {
     ...
     */
    encryptFile(path, password,userpath) {
        const salt = randomBytes(saltLen);
        const iv = randomBytes(ivLen);
        try {
            const _keys = keys.call(this, salt, password);
            ...
            const newPath = userpath + arr_path[arr_path.length-1]+".enc";
            // End my change
     ...
     * @return {String} The new file path.
     */
    decryptFile(path, password, userpath) {
        try {
            ...
            // Start my change
            const mac_check_failed = verifyFile(path,mac,macKey);
            if(!mac_check_failed){
                return false;
            }
            // End my changeverifyFile
